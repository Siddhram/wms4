...
export default function ActivatedWarehousePage() {
  ...
  const getWarehouseStatus = (warehouseCode: string): 'pending' | 'submitted' | 'activated' | 'rejected' | 'resubmitted' | 'closed' => {
    ...
    return 'pending'; // This will show no results for activated status
  };
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `activated-warehouse-inspections-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const convertInspectionToFormData = (inspection: InspectionData) => {
    ...
    return {
      ...
      status: 'activated', // Always activated for this page
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-3xl font-bold tracking-tight text-orange-600 inline-block border-b-4 border-green-500 pb-2 px-6 py-3 bg-orange-100 rounded-lg">
              <CheckCircle className="inline mr-2 h-8 w-8 text-green-500" />
              Activated Warehouses
            </h1>
          </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-medium text-green-900">Activated Warehouses</h3>
              <p className="text-green-700 mt-1">
                Warehouses that are fully operational and have been activated for storage operations.
              </p>
            </div>
          </div>
        </div>
        ...
        {filteredInspections.length > 0 ? (
          <Card className="border-green-300">
            <CardHeader className="bg-green-50">
              ...
              <CardDescription className="text-green-600">
                All activated warehouse inspection surveys with their details and actions.
              </CardDescription>
            </CardHeader>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {hasActiveFilters ? 'No Results Found' : 'No Activated Inspections'}
            </h3>
            <p className="text-gray-500">
              {hasActiveFilters 
                ? 'Try adjusting your search criteria or filters to find more results.'
                : 'There are currently no warehouse inspections in activated status.'
              }
            </p>
            {hasActiveFilters && (
              <Button
